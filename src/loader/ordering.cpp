...
// Where a card sits, as the tuple the comparison below reads in order
struct card_order
{
    ...
    // suit's sequence does not name. Both follow everything that is ranked
    int unranked = 0;
    ...
    // Member order is the comparison order, which is why it reads top to bottom
    // as the paragraphs of §4.3.2 do
...
std::vector<std::string> suit_order_of(std::vector<suit_info> const& suits)
{
...
card_order order_of(
    card const& c, std::vector<std::string> const& suit_order, std::vector<suit_info> const& suits
)
{
    if (c.id.is_major())
    {
...
void sort_cards(std::vector<card>& cards, std::vector<suit_info> const& suits)
{
    ...
    // Decorated, so that each card's key is built once rather than at every
    // comparison
    ...
    for (std::size_t i = 0; i < decorated.size(); ++i) cards[i] = std::move(decorated[i].second);
}
