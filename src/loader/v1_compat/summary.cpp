...
std::size_t count_cards(toml::table const& document)
{
    toml::table const& deck_table = *document["deck"].as_table();

    return count_standard_cards(get_string_array(deck_table["excluded_cards"]["cards"])) +
           count_custom_cards(document);
}
