...
//
// Every lookup on a catalog that did not load returns nullopt, so a deck with no
// names/ directory falls back to canonical names everywhere without the caller
// having to check.
class name_catalog
{
  public:
