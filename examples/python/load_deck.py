...
def main(argv: list[str]) -> int:
    ...
    try:
        # A deck is loaded by its directory name, not by [deck].identifier:
        # identifiers are not unique across a library, and find_all_by_id()
        # returns every deck carrying one.
        #
    ...
    print("  id            ", metadata.identifier or "(unset)")
    ...
    for suit in deck.suits:
        ...
        print(f"  suit {suit.key:10} {suit.name:12} {len(cards):2} cards{excluded}")
