...
// Unit tests for deck_loader, driven by inline deck.toml text rather than by fixture
// directories on disk. Only the tests that are about assets need a real directory.
...
// A path that does not exist, for the tests that are purely about parsing: every
// filesystem probe the loader makes against it fails harmlessly.
std::filesystem::path const no_deck_dir{"/nonexistent/arcana-deck"};
...
tags = ["classic", 7, "rider"]
    ...
    // A non-string element is skipped rather than defaulted in
...
name = "Plain"
    ...
    // The declared one keeps its place and its name; discovered ones follow, sorted
