...
constexpr std::string_view german = R"([major_arcana]
00 = "Der Narr"
)";
...
TEST_CASE("the requested language wins when it is present", "[names]")
{
    ...
    deck.write("names/de.toml", german);

    auto const names = name_catalog::load(deck.path(), "de");

    REQUIRE(names.loaded());
    CHECK(names.lookup("major_arcana", "00") == "Der Narr");
}
...
TEST_CASE("lookups miss without failing", "[names]")
{
    ...
    CHECK_FALSE(names.lookup("no_such_section", "00").has_value());
    CHECK_FALSE(names.lookup_minor("minor_arcana", "stars", "ace").has_value());

    // A section is a table, not a string, so asking for it as one misses
